<?php
/**
 * Automatic.css WooCommerce class file.
 *
...
/**
 * Automatic.css WooCommerce class.
 */
class WooCommerce extends Base implements Platform {
	...
	public function __construct( $is_enabled ) {
		...
		$this->css_file = $this->add_css_file(
			new CSS_File(
				'automaticcss-woocommerce',
				'automatic-woocommerce.css',
				array(
					'source_file' => 'platforms/woocommerce/automatic-woocommerce.scss',
					'imports_folder' => 'platforms/woocommerce',
				),
				array(
					'deps' => apply_filters( 'automaticcss_woocommerce_deps', array( 'woocommerce-general' ) ),
				)
			)
		);
		if ( is_admin() ) {
	...
	public function update_status( $variables ) {
		$woocommerce_enabled = isset( $variables['option-woocommerce'] ) && 'on' === $variables['option-woocommerce'] ? true : false;
		Logger::log( sprintf( '%s: setting the WooCommerce module to %s', __METHOD__, $variables['option-woocommerce'] ) );
		$this->set_enabled( $woocommerce_enabled );
		$this->css_file->set_enabled( $woocommerce_enabled );
	}
	...
	public static function is_active() {
		// I checked with class_exists( 'woocommerce' ), but it doesn't work here.
